...
module.exports =
    ...
    getGcodeMetadata: (slicer, gcode = null) ->
        ...
        # Find the metadata block (starts with "; Generated by Polyslice")
        ...
        for line in lines
            ...
            # If we haven't started metadata block yet, skip
            continue if not metadataStarted

            # Stop processing when we hit a non-comment line or empty line
            ...
            if line.startsWith(";")
                ...
                if colonIndex > 0
                    ...
                    camelKey = key.split(/\s+/).map((word, index) ->
                        if index is 0
                            word.toLowerCase()
                        else
                    ...
                    # Determine how to parse the value:
                    # 1. Plain integer: "50" -> 50
                    # 2. Plain float: "0.2" -> 0.2
                    # 3. Number with unit: "200°C" -> {value: 200, unit: "°C"}
                    # 4. Everything else (version numbers, timestamps, URLs, etc.): store as string

                    # Exclude version numbers (e.g., "1.0.0"), timestamps, and complex time formats
                    # Check for patterns that indicate it's NOT a simple numeric value
                    isVersionNumber = /^\d+\.\d+\.\d+/.test(value) # e.g., "1.0.0"
                    isTimestamp = /^\d{4}-\d{2}-\d{2}/.test(value) # e.g., "2024-01-01T..."
                    isComplexTime = /\d+[hms]\s+\d+[hms]/.test(value) # e.g., "1h 23m"

                    if isVersionNumber or isTimestamp or isComplexTime
                    ...
                    else if /^(\d+\.\d+|\d+)\s*([a-zA-Z°³].*)$/.test(value)

                        # Number with unit: "200°C" -> {value: 200, unit: "°C"}
                        # Match either float (0.78) or integer (200), then unit
                        match = value.match(/^(\d+\.\d+|\d+)\s*([a-zA-Z°³].*)$/)
                        ...
                        # Store as string (everything else)
                        metadata[camelKey] = value
