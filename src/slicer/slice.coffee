    ...
    generateLayerGCode: (slicer, paths, z, layerIndex, centerOffsetX = 0, centerOffsetY = 0, totalLayers = 0, allLayers = [], layerSegments = []) ->
        ...
        for pathIndex in sortedOuterBoundaryIndices
            ...
            if canUseSequentialCompletion
                ...
                # Check if only outer wall was generated by comparing wall count with expected walls.
                if wallCount > 1 and pathsWithInsufficientSpacingForInnerWalls[pathIndex]
                    # Thin wall case (insufficient spacing between paths).
                    infillBoundary = currentPath
                else
                    # Normal case: try to create inset for infill boundary.
                    infillBoundary = pathsUtils.createInsetPath(currentPath, nozzleDiameter, false)
                    # If inset fails (thin single path), use current path.
