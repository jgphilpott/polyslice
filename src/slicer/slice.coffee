                        ...
                        for holeIdx in [0...paths.length]
                            ...
                            if helpers.pointInPolygon(paths[holeIdx][0], currentPath)
                                ...
                                if checkSegments? and checkSegments.length > 0
                                    ...
                                    # If hole has closed, the parent path (outer boundary) is exposed.
                                    # Use the ORIGINAL outer path, not the innermost wall, to ensure there's room for skin wall inset.
                                    if not holeStillExists

                                        exposedAreas.push(paths[pathIndex])

                                        # Generate the hole's skin wall explicitly to create the inner ring boundary.
                                        # On middle layers, hole skin walls aren't generated by default.
                                        # We need both the hole's skin wall (inner ring) and the outer skin wall (outer ring).
                                        if innermostWalls[holeIdx] and innermostWalls[holeIdx].length >= 3

                                            # Calculate skin wall path for the hole
                                            skinWallInset = nozzleDiameter
                                            holeSkinWall = helpers.createInsetPath(innermostWalls[holeIdx], skinWallInset, true)

                                            if holeSkinWall and holeSkinWall.length >= 3

                                                # Generate G-code for the hole's skin wall (draw it)
                                                # This creates the inner boundary of the ring
                                                # Pass isHole=true, generateInfill=false (only wall, no infill)
                                                # Do NOT add to holeSkinWalls yet - that would clip the outer wall
                                                skinModule.generateSkinGCode(slicer, innermostWalls[holeIdx], z, centerOffsetX, centerOffsetY, layerIndex, lastPathEndPoint, true, false, [], holeOuterWalls)

                                                # NOW add it to clipping array for the infill generation
                                                holeSkinWalls.push(holeSkinWall)
